...
export interface AgentInfo { name: string; welcome: string }
export interface PendingEscalation {
  id: string; order_id: string; amount: number | null; reason: string; created_at: string
}
export interface SessionDetail {
  messages: any[]; logs: AgentLog[]; escalations: PendingEscalation[]
}
...
export const api = {
  agent: () => getJson<AgentInfo>(`/api/v1/agent`),
  ...
  resolveEscalation: (
    id: string, action: 'approved' | 'rejected', reason: string, reviewer = 'admin',
  ) =>
    postJson<{ resolution: string; reply: string }>(
      `/api/v1/admin/escalations/${id}/resolve`, { action, reason, reviewer },
    ),
}
